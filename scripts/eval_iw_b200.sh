...
resolve_run_paths
MODEL_PATH="${IW_CHECKPOINT:-${IW_RUN_OUTPUT}/final}"
EVAL_OUTPUT="${IW_EVAL_OUTPUT:-${RUN_RESULTS_DIR}/eval/iw}"
require_file "${MODEL_PATH}/config.json"
cd "${REPO_DIR}"
exec "${PYTHON_BIN}" -m b200_experiment.cli evaluate \
  --config "${IW_CONFIG}" --name "IW-OPD" --model "${MODEL_PATH}" \
  "${ASSET_CONFIG_ARGS[@]}" \
  --output "${EVAL_OUTPUT}" \
  ...
  --set "evaluation.max_new_tokens=${EVAL_MAX_NEW_TOKENS:-4096}" \
  ...
  --set "evaluation.vllm.max_model_len=${EVAL_VLLM_MAX_MODEL_LEN:-5120}" \
