...
def _opd_train_step(
    ...
    for ppo_index in range(offset, min(ppo_count, offset + remaining_limit)):
        ...
        analysis_step = optimizer_step_start + len(minibatch_metrics) + 1
        if analysis_session is not None:
            analysis_session.before_optimizer_step(
                analysis_step, model, indices, ppo_valid, ppo_weights
            )
        ...
        for chunk_index, (chunk_indices, chunk_active) in enumerate(
            ...
            with sync_context:
                ...
                if analysis_session is not None:
                    analysis_session.on_training_chunk(
                        analysis_step,
                        chunk_indices,
                        per_position_loss,
                        chunk_weights,
        ...
        if analysis_session is not None:
            analysis_session.after_optimizer_step(
                analysis_step, model, float(gradient_norm.detach().item())
            )
        optimizer.zero_grad(set_to_none=True)
...
def run_training(
    ...
    if bool(config.get("analysis", {}).get("enabled", False)) and method != "cmt":
        raise ValueError("analysis.enabled currently requires experiment.method=cmt")
    if method == "grpo":
    ...
    while optimizer_step < max_steps:
        ...
        analysis_session = (
            analysis_logger.begin_rollout(
                ...
                first_optimizer_step=rollout_first_optimizer_step,
                rollout=rollout,
                selector=primary,
        ...
        if analysis_logger is not None:
            analysis_logger.finish_rollout(analysis_session)
        if method == "cmt":
    ...
    summary = {
        ...
        "analysis_dir": str(analysis_logger.root) if analysis_logger is not None else None,
        "evaluation_history": str((output_dir / "eval_history.jsonl").resolve())
        if bool(training_eval_settings.get("enabled", False))
        else None,
        "initial_evaluation": initial_evaluation,
